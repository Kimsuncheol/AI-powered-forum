...
export default function FollowingUserAvatars() {
  const { user } = useAuth();
  const { users, loading, loadingMore, hasMore, loadMore } = useInfiniteFollowingUsers(user?.uid);
  ...
  if (loading && users.length === 0) {
    return (
      <Box sx={{ display: "flex", gap: 2, overflowX: "auto", pb: 2, mb: 2 }}>
        {[...Array(6)].map((_, i) => (
          <Skeleton key={i} variant="circular" width={56} height={56} sx={{ flexShrink: 0 }} />
        ))}
      </Box>
    );
  }
  ...
  if (users.length === 0) {
    // If no following users, strictly return null or maybe an empty, collapsed box.
    // The requirement was to place it at the "Welcome Header" place.
    // If empty, maybe just nothing.
    return null;
  }
  ...
  return (
    <Box sx={{ mb: 3 }}>
        ...
        {loadingMore && hasMore && (
            <Box sx={{ display: "flex", alignItems: "center", justifyContent: "center", width: 64, height: 64, flexShrink: 0 }}>
